...
public class AnimalController {
  ...
  public AnimalController(
      ...
      AnimalModelAssembler animalModelAssembler,
  ...
  public CollectionModel<EntityModel<Animal>> getAllAnimals() {
    List<EntityModel<Animal>> animals =
        animalRepository.findAll().stream()
            .map(animalModelAssembler::toModel)
            .collect(Collectors.toList()); // displays Animals with self links to make data RESTful

    return CollectionModel.of(
        animals, linkTo(methodOn(AnimalController.class).getAllAnimals()).withSelfRel());
  }

  @GetMapping("/animals/id/{id}")
  public EntityModel<Animal> getOneAnimal(@PathVariable Long id) {
    Animal animal =
        animalRepository.findById(id).orElseThrow(() -> new AnimalNotFoundException(id));
  ...
  public CollectionModel<EntityModel<Animal>> getOneAnimal(@PathVariable String name) {
    List<EntityModel<Animal>> animals =
        animalRepository.findByName(name).stream()
            .map(animalModelAssembler::toModel)
    ...
    return CollectionModel.of(
        animals,
        linkTo(methodOn(AnimalController.class).getOneAnimal(name)).withRel("animals " + name));
  }
